...
module.exports = async (req, res) => {
  if (!req.file) return error(res, 'No PowerPoint file uploaded');
  ...
  try {
    await runLibreOffice(['--convert-to', 'pdf:impress_pdf_Export', '--outdir', outputDir, inputPath]);
    ...
    success(res, { message: 'PowerPoint converted to PDF', filename: outName, downloadUrl: `${downloadUrl(req, outName)}?name=PDF Tools` });
  } catch (err) {
    if (fs.existsSync(convertedPath)) fs.unlinkSync(convertedPath);
    deleteFile(inputPath);
