...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (buf[0] !== 0x50 || buf[1] !== 0x4B) { fs.unlinkSync(convertedPath); throw new Error('Output is not a valid DOCX.'); }
    const outName = `${uuidv4()}.docx`;
    fs.renameSync(convertedPath, path.join(outputDir, outName));
    deleteFile(inputPath);
    success(res, { message: 'PDF converted to DOCX', filename: outName, downloadUrl: `${downloadUrl(req, outName)}?name=PDF Tools` });
  } catch (err) {
    if (fs.existsSync(convertedPath)) fs.unlinkSync(convertedPath);
    deleteFile(inputPath);
